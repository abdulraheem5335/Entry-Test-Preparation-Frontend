...
export default function Navbar() {
  ...
  const navLinkClasses = ({ isActive }) =>
    `font-medium transition-colors hover:text-blue-600 ${isActive ? 'text-blue-600' : 'text-gray-700'
    }`;

  const mobileNavLinkClasses = ({ isActive }) =>
    `block py-2 text-lg ${isActive ? 'text-blue-600 font-semibold' : 'text-gray-700'
    }`;
              ...
              {isLoggedIn ? (
                <>
                  ...
                  >
                    ...
                    />
                    <div
                      className={`absolute top-full right-0 mt-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg py-1 transition-all duration-300 ease-in-out ${isProfileOpen
                        ? 'opacity-100 visible translate-y-0'
                        : 'opacity-0 invisible -translate-y-2'
                        }`}
                    >
                      <NavLink to="/profile" className="block px-4 py-2 text-gray-700 hover:bg-gray-100">My Profile</NavLink>
                      <NavLink to="/profile/settings" className="block px-4 py-2 text-gray-700 hover:bg-gray-100">Settings</NavLink>
