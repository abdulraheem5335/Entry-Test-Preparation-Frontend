...
const subjectChapters = {
    ...
    physics: {
        ...
        description: "Understand the laws of physics and their applications",
        chapters: [
    ...
    analytical: {
        ...
        chapters: [
            { id: 1, title: "Logical Sequences", topics: 6, time: "1h 30m", status: "completed", locked: false },
            { id: 2, title: "Pattern Recognition", topics: 8, time: "2h", status: "in-progress", locked: false },
            { id: 3, title: "Critical Reasoning", topics: 7, time: "2h", status: "not-started", locked: false },
        ]
    },
    quantitative: {
        ...
        chapters: [
            { id: 1, title: "Number Systems", topics: 5, time: "1h 15m", status: "completed", locked: false },
            { id: 2, title: "Data Interpretation", topics: 8, time: "2h", status: "in-progress", locked: false },
            { id: 3, title: "Word Problems", topics: 10, time: "2h 30m", status: "not-started", locked: false },
        ]
    }
};
...
const ChapterSelection = () => {
    ...
    const themeColor = examType === 'mdcat' ? 'emerald' : examType === 'nat' ? 'violet' : 'blue';
    ...
    const filteredChapters = useMemo(() => {
        ...
        return subjectData.chapters.filter(c => c.title.toLowerCase().includes(searchQuery.toLowerCase()));
    }, [subjectData, searchQuery]);

    const getStatusStyles = (status) => {
        switch (status) {
            case "completed": return { bg: "bg-emerald-50", border: "border-emerald-200", text: "text-emerald-700", icon: "bg-emerald-500" };
            case "in-progress": return { bg: `bg-${themeColor}-50`, border: `border-${themeColor}-200`, text: `text-${themeColor}-700`, icon: `bg-${themeColor}-500` };
            default: return { bg: "bg-slate-50", border: "border-slate-200", text: "text-slate-600", icon: "bg-slate-300" };
        }
    };
                ...
                <div className="bg-white border-b border-slate-200">
                    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
                        <nav className="mb-6 flex items-center gap-2 text-sm text-slate-500 flex-wrap">
                            <Link to="/" className={`hover:text-${themeColor}-600 transition`}>Home</Link>
                            <span className="text-slate-300">/</span>
                            <Link to={`/${examType}`} className={`hover:text-${themeColor}-600 transition`}>{examType.toUpperCase()}</Link>
                            <span className="text-slate-300">/</span>
                            <Link to={`/${examType}/${stream}`} className={`hover:text-${themeColor}-600 transition`}>{formatName(stream)}</Link>
                            <span className="text-slate-300">/</span>
                            <span className="text-slate-900 font-medium">{subjectData.name}</span>
                        </nav>

                        <div className="flex items-start gap-6">
                            <img src={subjectData.image} alt={subjectData.name} className="w-20 h-20 rounded-xl object-cover border border-slate-200" />
                            <div>
                        ...
                        <div>
                            <div className="relative mb-6">
                                <input
                                    ...
                                    className={`w-full pl-10 pr-4 py-2.5 rounded-lg border border-slate-200 bg-white focus:ring-2 focus:ring-${themeColor}-500 focus:border-transparent transition text-sm`}
                                />
                            ...
                            <div className="space-y-3">
                                {filteredChapters.map((chapter) => {
                                    ...
                                    return (
                                        <article key={chapter.id} className={`bg-white rounded-xl border ${chapter.locked ? 'opacity-60' : 'hover:border-blue-300'} border-slate-200 transition-all`}>
                                            <div className="p-5">
                                                <div className="flex items-start gap-4">
                                                    <div className={`w-10 h-10 rounded-full ${chapter.status === 'completed' ? 'bg-emerald-500' : chapter.status === 'in-progress' ? 'bg-blue-500' : 'bg-slate-300'} flex items-center justify-center flex-shrink-0`}>
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <div className="flex items-start justify-between gap-4 mb-2">
                                                            <h3 className="text-base font-semibold text-slate-900">Chapter {chapter.id}: {chapter.title}</h3>
                                                            {chapter.locked && (
                                                        ...
                                                        <div className="flex items-center gap-4 text-sm text-slate-500 mb-3">
                                                            ...
                                                            <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${chapter.status === 'completed' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : chapter.status === 'in-progress' ? 'bg-blue-50 text-blue-700 border border-blue-200' : 'bg-slate-50 text-slate-600 border border-slate-200'}`}>
                                                                {chapter.status === "completed" ? "Completed" : chapter.status === "in-progress" ? "In Progress" : "Not Started"}
                                                            </span>
                                                        </div>

                                                        {chapter.locked ? (
                                                            <button disabled className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-slate-100 text-slate-400 text-sm cursor-not-allowed">
                                                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                                                                </svg>
                                                                Locked
                                                            ...
                                                            <Link
                                                                to={`/${examType}/${stream}/${subject}/chapter-${chapter.id}`}
                            ...
                            <div className="bg-white rounded-xl border border-slate-200 p-6 sticky top-24">
                                ...
                                <div className="mb-6">
                                    ...
                                    <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                                        <div className="h-full bg-blue-600 rounded-full transition-all" style={{ width: `${(completedChapters / subjectData.chapters.length) * 100}%` }} />
                                    </div>
                                </div>
